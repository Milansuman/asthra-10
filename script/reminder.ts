...
//     subject: 'Mark your attendance -  ASTHRA 9',
...
for (const user of allUSer) {
    const { isSuccess, data:isAttented } = await triedAsync(api.management.isAttendedAny.query({
      id: user.id,
    }));
    ...
    if (isAttented && isAttented.length > 0) {
      sentMail({
          to: user.email,
          subject: 'Certificate Ready -  ASTHRA 9',
          text: "Thank you for registering & attending ASTHRA 9. We appreciate your active participation. Also your certificate is ready. Please download from your profile. \n\n Link: https://asthra.sjcetpalai.ac.in/profile",
        })
